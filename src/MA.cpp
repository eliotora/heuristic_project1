...
void MA::recombination() {
    for (int i=0; i<sp.size(); i++) {
        ...
        vector<int> child_genome = cross_over_operator(genome1, genome2);
...
vector<int> MA::cross_over_operator(vector<int> genome1, vector<int> genome2) {
    ...
    vector<int> repairs = vector<int>(genome2.begin(), genome2.end()); // Part of the genome2 that won't be in the child

    for (int gene: g1) { // remove from g2 and repairs the genes that are in g1
        g2.erase(std::remove(g2.begin(), g2.end(), gene));
        repairs.erase(std::remove(repairs.begin(), repairs.end(), gene));
    }
...
vector<int> MA::get_best_solution() {
    return max_element(sp.begin(), sp.end(), compFlowInst)->getCurrentSolution();
}

long MA::get_best_score() {
    return max_element(sp.begin(), sp.end(), compFlowInst)->getCurrentScore();
}